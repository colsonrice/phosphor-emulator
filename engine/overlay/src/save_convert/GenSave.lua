...
-- The map-header cache (wCurrentTileBlockMapViewPointer .. the connection
-- headers) plus wDestinationWarpID. On CONTINUE the real game TRUSTS these
-- bytes instead of rebuilding them from ROM: LoadMapHeader returns early on
-- the continue path (home/overworld.asm, the BIT_NO_PREVIOUS_MAP check), and
-- the overworld loop's very first RunMapScript does `jp [wMapScriptPtr]`
-- after banking to wCurMap's bank. A template-filled window therefore
-- executes the TEMPLATE map's script pointer inside the MODELED map's bank —
-- data as code, rst $38, black screen. Proven live: an export saved outdoors
-- in Cerulean City carried the previous export's CeruleanPokecenter_Script
-- pointer ($4501, bank $17) and crashed the cartridge in bank $06; the same
-- save exported inside the Pokécenter "worked" only because template and
-- position happened to agree. encode() derives the whole window from the ROM
-- for the modeled position (see deriveMapMachineWindow).
O.tileBlockMapViewPtr = O.mainData + 104                  -- 2B LE (wCurrentTileBlockMapViewPointer)
O.yBlockCoord = O.mainData + 108                          -- 1B (wYCoord & 1 — LoadTilesetHeader's rule)
O.xBlockCoord = O.mainData + 109                          -- 1B
O.curMapTileset = O.mainData + 112                        -- 1B
O.curMapHeight = O.mainData + 113                         -- 1B (blocks)
O.curMapWidth = O.mainData + 114                          -- 1B (blocks)
O.mapDataPtr = O.mainData + 115                           -- 2B LE
O.mapTextPtr = O.mainData + 117                           -- 2B LE
O.mapScriptPtr = O.mainData + 119                         -- 2B LE
O.mapConnections = O.mainData + 121                       -- 1B (NORTH_F=3 SOUTH_F=2 WEST_F=1 EAST_F=0)
O.connNorth = O.mainData + 122                            -- 11B fixed slot (wNorthConnectionHeader)
O.connSouth = O.mainData + 133                            -- 11B
O.connWest = O.mainData + 144                             -- 11B
O.connEast = O.mainData + 155                             -- 11B
O.destinationWarpID = O.mainData + 312                    -- 1B (wDestinationWarpID; $FF = none)
-- LoadMapHeader's early return also skips its whole second half — the object
-- data (border block, warps, signs, people-sprites), the 11-byte tileset
-- header, the wild slots, the doubled dims and the map music — so continue
-- trusts ALL of it from the save. Offsets are R/B WRAM minus wMainDataStart
-- (the save layout is shared by all three versions).
O.mapMusicSoundID = O.mainData + 100                      -- 1B (wMapMusicSoundID D35B)
O.mapMusicROMBank = O.mainData + 101                      -- 1B
O.mapBackgroundTile = O.mainData + 182                    -- 1B (border block)
O.numberOfWarps = O.mainData + 183                        -- 1B
O.warpEntries = O.mainData + 184                          -- 32 x 4B (y, x, destWarpId, destMap)
O.numSigns = O.mainData + 441                             -- 1B
O.signCoords = O.mainData + 442                           -- 16 x 2B (y, x)
O.signTextIDs = O.mainData + 474                          -- 16 x 1B
O.numSprites = O.mainData + 490                           -- 1B (wNumSprites D4E1)
O.mapSpriteData = O.mainData + 493                        -- 16 x 2B (movement byte 2, text id)
O.mapSpriteExtraData = O.mainData + 525                   -- 16 x 2B (trainer class/num or item id)
O.currentMapHeight2 = O.mainData + 557                    -- 1B (height in 2x2 blocks = 2*h)
O.currentMapWidth2 = O.mainData + 558                     -- 1B
O.tilesetHeader = O.mainData + 564                        -- 11B (wTilesetBank..: bank, blocks, gfx, collision ptrs, talking-over tiles, grass tile)
O.grassRate = O.mainData + 1424                           -- 1B (wGrassRate D887)
O.grassMons = O.mainData + 1425                           -- 20B
O.waterRate = O.mainData + 1453                           -- 1B
O.waterMons = O.mainData + 1454                           -- 20B
...
-- Rebuild the map-header cache window for the save's modeled position from
-- the actual cartridge, byte-for-byte what LoadMapHeader would produce:
-- tileset/height/width, the data/text/script pointers, the connections mask
-- and the four fixed connection-header slots (absent directions get the
-- loader's own disabled state: $FF map id, zeroed strip). The view pointer
-- is computed from the position — wOverworldMap ($C6E8) + (width+6) ×
-- (y>>1 + 1) + (x>>1 + 1), validated byte-exact against two real saves
-- (Viridian outdoor y28 x4 w20 → $C871; Cerulean Pokécenter y4 x13 w7 →
-- $C716) — with the block parities LoadTilesetHeader derives (coord & 1),
-- and wDestinationWarpID is cleared to $FF, the "exclude normal warps"
-- value the game's own special-warp path writes.
--
-- `rom` = { bytes = <full cartridge image>, pointers = <file offset of
-- MapHeaderPointers>, banks = <file offset of MapHeaderBanks> } — the
-- table offsets differ per version, so SaveConvert resolves them.
-- Returns true when the window was derived; false leaves the template
-- bytes untouched.
function GenSave.deriveMapMachineWindow(buf, save, cw, rom)
  if type(rom) ~= "table" or type(rom.bytes) ~= "string" then return false end
  local mapIdx = save.player and save.player.map and cw.mapsIndex[save.player.map]
  if not mapIdx then return false end
  local r = rom.bytes
  local function rb(off)                     -- 0-based file offset
    if off < 0 or off + 1 > #r then return nil end
    return r:byte(off + 1)
  end
  local bank = rb(rom.banks + mapIdx)
  local ptrLo = rb(rom.pointers + mapIdx * 2)
  local ptrHi = rb(rom.pointers + mapIdx * 2 + 1)
  if not (bank and ptrLo and ptrHi) then return false end
  local ptr = ptrLo + ptrHi * 256
  if ptr < 0x4000 or ptr >= 0x8000 then return false end
  local hdr = bank * 0x4000 + (ptr - 0x4000)
  if not rb(hdr + 9) then return false end
  -- fixed header: tileset, height, width, dataPtr, textPtr, scriptPtr,
  -- connections — one contiguous run, same order in ROM and WRAM
  for i = 0, 9 do setByte(buf, O.curMapTileset + i, rb(hdr + i)) end
  -- connection headers ride the ROM in check order (N, S, W, E), 11 bytes
  -- each, present only when the mask bit says so; the WRAM slots are fixed
  local mask = rb(hdr + 9)
  local off = hdr + 10
  local slots = {
    { flag = 3, base = O.connNorth },   -- NORTH_F
    { flag = 2, base = O.connSouth },   -- SOUTH_F
    { flag = 1, base = O.connWest },    -- WEST_F
    { flag = 0, base = O.connEast },    -- EAST_F
  }
  for _, s in ipairs(slots) do
    if bit.band(mask, bit.lshift(1, s.flag)) ~= 0 then
      for i = 0, 10 do setByte(buf, s.base + i, rb(off + i) or 0) end
      off = off + 11
    else
      setByte(buf, s.base, 0xFF)
      for i = 1, 10 do setByte(buf, s.base + i, 0) end
    end
  end
  local width = rb(hdr + 2)
  local height = rb(hdr + 1)
  local y = (save.player and save.player.y) or 0
  local x = (save.player and save.player.x) or 0
  local vp = 0xC6E8 + (width + 6) * (math.floor(y / 2) + 1) + (math.floor(x / 2) + 1)
  setByte(buf, O.tileBlockMapViewPtr, bit.band(vp, 0xFF))
  setByte(buf, O.tileBlockMapViewPtr + 1, bit.band(bit.rshift(vp, 8), 0xFF))
  setByte(buf, O.yBlockCoord, bit.band(y, 1))
  setByte(buf, O.xBlockCoord, bit.band(x, 1))
  setByte(buf, O.destinationWarpID, 0xFF)
  setByte(buf, O.currentMapHeight2, bit.band(height * 2, 0xFF))
  setByte(buf, O.currentMapWidth2, bit.band(width * 2, 0xFF))

  -- LoadMapHeader's second half, mirrored: the object data pointer follows
  -- the connection headers in the ROM (off already walked past them).
  local objPtr = rb(off) + rb(off + 1) * 256
  if objPtr >= 0x4000 and objPtr < 0x8000 then
    local obj = bank * 0x4000 + (objPtr - 0x4000)
    setByte(buf, O.mapBackgroundTile, rb(obj) or 0)
    local p = obj + 1
    local nWarps = math.min(rb(p) or 0, 32)
    setByte(buf, O.numberOfWarps, nWarps)
    p = p + 1
    for i = 0, nWarps * 4 - 1 do
      setByte(buf, O.warpEntries + i, rb(p + i) or 0)
    end
    p = p + nWarps * 4
    local nSigns = math.min(rb(p) or 0, 16)
    setByte(buf, O.numSigns, nSigns)
    p = p + 1
    for i = 0, nSigns - 1 do
      setByte(buf, O.signCoords + i * 2, rb(p) or 0)
      setByte(buf, O.signCoords + i * 2 + 1, rb(p + 1) or 0)
      setByte(buf, O.signTextIDs + i, rb(p + 2) or 0)
      p = p + 3
    end
    -- People-objects, expanded exactly as InitSprites + LoadSprite do:
    -- slots 1..14 zeroed with image index $FF, then per object j the pic id
    -- into sprite-state page 1, map y/x/movement-1 into page 2, movement-2
    -- and text id into wMapSpriteData, trainer/item payloads into
    -- wMapSpriteExtraData. Slot 15 (Yellow's Pikachu) and slot 0 (the
    -- player) keep their template bytes — the player slot is reset by the
    -- continue path itself, and the follower re-anchors to the player.
    local S1 = 11564                -- sav offset of C100 (sprite page 1)
    local S2 = 11564 + 256         -- C200 (sprite page 2)
    for slot = 1, 14 do
      for b = 0, 15 do
        setByte(buf, S1 + slot * 16 + b, 0)
        setByte(buf, S2 + slot * 16 + b, 0)
      end
      setByte(buf, S1 + slot * 16 + 2, 0xFF)   -- image index: disabled
    end
    local nSprites = math.min(rb(p) or 0, 14)
    setByte(buf, O.numSprites, nSprites)
    p = p + 1
    for j = 0, nSprites - 1 do
      local slot = j + 1
      local pic = rb(p) or 0
      local mapY = rb(p + 1) or 0
      local mapX = rb(p + 2) or 0
      local mov1 = rb(p + 3) or 0
      local mov2 = rb(p + 4) or 0
      local textFlags = rb(p + 5) or 0
      p = p + 6
      setByte(buf, S1 + slot * 16 + 0, pic)
      setByte(buf, S2 + slot * 16 + 4, mapY)
      setByte(buf, S2 + slot * 16 + 5, mapX)
      setByte(buf, S2 + slot * 16 + 6, mov1)
      setByte(buf, O.mapSpriteData + j * 2, mov2)
      setByte(buf, O.mapSpriteData + j * 2 + 1, bit.band(textFlags, 0x3F))
      local extraA, extraB = 0, 0
      if bit.band(textFlags, 0x40) ~= 0 then          -- BIT_TRAINER
        extraA, extraB = rb(p) or 0, rb(p + 1) or 0
        p = p + 2
      elseif bit.band(textFlags, 0x80) ~= 0 then      -- BIT_ITEM
        extraA = rb(p) or 0
        p = p + 1
      end
      setByte(buf, O.mapSpriteExtraData + j * 2, extraA)
      setByte(buf, O.mapSpriteExtraData + j * 2 + 1, extraB)
    end
  end

  -- The 11-byte tileset header (wTilesetBank..), from the Tilesets table.
  if rom.tilesets then
    local tileset = rb(hdr)
    for i = 0, 10 do
      setByte(buf, O.tilesetHeader + i, rb(rom.tilesets + tileset * 12 + i) or 0)
    end
  end

  -- Map music, from MapSongBanks (2 bytes per map: sound id, bank).
  if rom.songs then
    setByte(buf, O.mapMusicSoundID, rb(rom.songs + mapIdx * 2) or 0)
    setByte(buf, O.mapMusicROMBank, rb(rom.songs + mapIdx * 2 + 1) or 0)
  end

  -- Wild slots, exactly as LoadWildData fills them: rate then, when the
  -- rate is non-zero, the 20-byte slot table; a zero rate leaves the mons
  -- exactly as the game itself would (unread stale bytes).
  if rom.wild then
    local wptr = rb(rom.wild + mapIdx * 2) + (rb(rom.wild + mapIdx * 2 + 1) or 0) * 256
    if wptr >= 0x4000 and wptr < 0x8000 then
      local w = 0x3 * 0x4000 + (wptr - 0x4000)   -- WildDataPointers lives in bank 3
      local grassRate = rb(w) or 0
      setByte(buf, O.grassRate, grassRate)
      w = w + 1
      if grassRate ~= 0 then
        for i = 0, 19 do setByte(buf, O.grassMons + i, rb(w + i) or 0) end
        w = w + 20
      end
      local waterRate = rb(w) or 0
      setByte(buf, O.waterRate, waterRate)
      if waterRate ~= 0 then
        for i = 0, 19 do setByte(buf, O.waterMons + i, rb(w + 1 + i) or 0) end
      end
    end
  end
  return true
...
function GenSave.encode(save, data, template, rom)
  ...
  -- The map-header cache MUST agree with the position just written (see the
  -- offset-table comment). With ROM bytes available the whole window is
  -- derived for the modeled map, exactly as LoadMapHeader would build it;
  -- without them the template bytes stand, which is only safe when the
  -- template's map is the same map — the pre-fix behavior.
  GenSave.deriveMapMachineWindow(buf, save, cw, rom)
