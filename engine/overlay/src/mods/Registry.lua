...
-- the owner Builtins.install registers engine content under.  Restated
-- here as a literal rather than required from Schemas so Registry stays a
-- leaf module usable without the schema catalog
local ENGINE = "engine"
...
function Registry.new(name, spec)
  return setmetatable({
    ...
    skipped = {},  -- loser "\0" winner -> skip tally, summarized at freeze
    frozen = false,
  }, Registry)
end
...
function Registry:register(id, value, owner, replace)
  ...
  if self.spec.semantics == "record" and self:get(id) ~= nil then
    ...
    local current = self.owners[id]
    -- vanilla content (base table or engine-owned ops), ownerless v1/tool
    -- registries, and a mod colliding with itself keep the hard error:
    -- those are deterministic authoring bugs the author sees on their own
    -- machine, and replacing that content has to say override.  Two mods
    -- meeting over one id is a property of the player's install set, not a
    -- bug in either mod, so the first owner keeps the id and the later
    -- register degrades to an attributed warning -- erroring here used to
    -- kill the losing mod's entire load over one cosmetic slot
    if current == nil or current == ENGINE or owner == nil
        or current == owner then
      error(("%s already registered: %s"):format(self.name, id))
    end
    -- one full line per (loser, winner) pair; a mod colliding across a
    -- whole species table would otherwise flood the log ring, so the rest
    -- is tallied and summarized when the loader freezes content
    local key = owner .. "\0" .. current
    local tally = self.skipped[key]
    if tally then
      tally.count = tally.count + 1
    else
      self.skipped[key] = { owner = owner, current = current, count = 1 }
      Logger.warn("[%s] %s already registered: %s -- keeping %s's value; "
        .. "say override to replace it", tostring(owner), self.name, id,
        tostring(current))
    end
    return nil
  end
...
function Registry:rollback(owner)
  ...
  for key, tally in pairs(self.skipped) do
    if tally.owner == owner then self.skipped[key] = nil end
  end
...
-- stays deterministic by refusing registration after that point.  The
-- deferred skip tallies flush here, sorted so the log stays deterministic
function Registry:freeze()
  self.frozen = true
  local keys = {}
  for key, tally in pairs(self.skipped) do
    if tally.count > 1 then keys[#keys + 1] = key end
  end
  table.sort(keys)
  for _, key in ipairs(keys) do
    local tally = self.skipped[key]
    Logger.warn("[%s] %s: %d more ids kept %s's values", tally.owner,
      self.name, tally.count - 1, tally.current)
  end
  self.skipped = {}
end
