...
-- See src/mods/ModRenderGuard.lua.
pcall(ModRenderGuard.install)
...
function Loader.new(opts)
  ...
  local self = setmetatable({
    ...
    -- PHOSPHOR: installs whose manifest the validator refused, so an
    -- embedding host can say WHY instead of showing a mod that will never
    -- load as merely waiting. See _discover.
    rejected = {},
      ...
      for _, name in ipairs(self.fs.getDirectoryItems(root)) do
        ...
        if info and (info.type == "directory" or info.type == "symlink") then
          ...
          if manifest then
            ...
            -- PHOSPHOR: a refused manifest used to leave nothing but this
            -- warning, and on a phone nobody reads stdout. The embedding host
            -- lists mods from DISK, so such a mod sat on screen as "on,
            -- pending first launch" forever while the loader had already
            -- decided it would never run -- indistinguishable, to the player,
            -- from a mod that simply had not booted yet. Keep the rejection
            -- so HostSeam.writeModState can report it with its reason.
            --
            -- Keyed by DIRECTORY NAME, because the manifest's own id is
            -- exactly the thing that could not be trusted; that is also the
            -- name the host lists an unreadable install under, so the two
            -- agree on one row.
            --
            -- Deliberately NOT placed in self.mods: every later phase indexes
            -- mod.manifest, and a stub entry is a nil-index waiting for the
            -- next loop written without it in mind. A separate list cannot
            -- change how a single mod loads.
            self.rejected[#self.rejected + 1] =
              { id = name, path = path, reason = tostring(err) }
...
function Loader:_modEnv(mod)
  ...
  if not env then
    ...
    env = Sandbox.envFor({
      ...
      -- so it can only ever reach its own files.
      selfModules = { fs = self.fs, path = mod.path },
      -- PHOSPHOR: print() is a mod talking, and on a phone nobody reads
      -- stdout.  Same shape as api.log below, so a mod's print lands in the
      -- feed the mod manager shows, attributed to the mod that wrote it.
      log = {
        info = function(_, fmt, ...) Logger.info("[%s] " .. fmt, id, ...) end,
      },
    })
    self.modEnv[id] = env
  end
...
function Loader:legacyReport(modId)
  -- LegacyCompat is never constructed here (see _modEnv), so its report is
  -- always empty today; it stays in the union so this keeps answering
  -- correctly if that ever changes. PointerBridge is the one that has
  -- anything to say: which legacy pointer names a mod assigned.
  local report = LegacyCompat.report(modId) or {}
  local pointer = PointerBridge.report(modId)
  if pointer then
    for name in pairs(pointer) do report[name] = true end
  end
  return report
end

function Loader:_loadMod(mod)
  -- PHOSPHOR: refuse rather than run a mod unconfined.  Sandbox.bind returns
  -- nil on a Lua that can neither setfenv nor rewrite an _ENV upvalue, and
  -- "we could not sandbox it, so we ran it anyway" is the one outcome nobody
  -- would choose on purpose.  Mod code is code the player downloaded from the
  -- internet; on a sandboxed host, running it against the real globals is the
  -- difference between a game engine and an app that executes what it fetched.
  if not Sandbox.available() then
    error("this build cannot sandbox mod code; refusing to load " ..
          tostring(mod.manifest.id), 0)
  end
  ...
  -- Phosphor: a Gen 2 voxel world renders upside down on LOVE 12 unless the
  -- mod normalises clip space itself. Must run BEFORE the entry chunk, which
  -- is when the mod reads its lib files through api:read. Self-cancelling, and
  -- a no-op for every other mod -- see src/mods/Gen2ClipSpaceShim.lua.
  pcall(Gen2ClipSpaceShim.consider, self, mod, api)
