...
-- are dropped and the 32768-byte save imports.  extras is a SaveExtras sidecar
-- the host staged beside the save (see src/save_convert/SaveExtras.lua): when
-- its stamp matches these bytes it restores the bag and PC rows a 20-slot
-- cartridge bag had no room for, and otherwise it is ignored.
function SaveFileIO.importToSlot(source, version, force, extras)
  ...
  -- 32768 the exporter hashed -- so an RTC footer trimmed above cannot make a
  -- matching pair look like strangers. A sidecar for other bytes is ignored,
  -- and the import is then exactly what it was before this existed.
  if extras then
    SaveExtras.apply(save, extras, SaveExtras.stamp(bytes))
  end
