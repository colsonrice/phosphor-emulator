    ...
    if info and (info.type == "directory" or info.type == "symlink") then
      ...
      if okV and m and not seen[m.id] and m.options_schema then
        ...
        if enabled and not SaveData.isSafeMode(opts) then
          -- PHOSPHOR OVERLAY.  An options schema is MOD code like any other
          -- and has to run in the mod sandbox: loaded plainly, this chunk
          -- gets the real globals (ffi, io, require) purely because the
          -- settings gear read it rather than Loader:_loadMod.  "data-only"
          -- describes what the file is SUPPOSED to return, not what the
          -- engine is obliged to let it do -- it is still a Lua chunk out of
          -- a folder the player installed off the internet, and this runs on
          -- an ordinary launcher boot for every mod that declares one.
          -- Loader:_writeOptionSchemas and ManagerState:schemaFor are the
          -- other two sites and upstream sandboxes both; this third one is
          -- still unguarded upstream at this pin, which is the whole reason
          -- this file is in the overlay.  tests/mod_sandbox_tests.lua fails
          -- if a fourth appears.  Fails CLOSED: a chunk that cannot be
          -- confined contributes no row.
          --
          -- The SAFE MODE half of the condition is upstream's (0.2.9): a
          -- player who booted with mods off must not have one's schema
          -- chunk run either, sandboxed or not.
          local okR, schema = pcall(function()
            local Sandbox = require("src.mods.Sandbox")
            local SafePath = require("src.mods.SafePath")
            local rel = SafePath.join(path, m.options_schema, "options_schema")
            local chunk = Sandbox.loadFile(fs, rel, Sandbox.envFor({ modId = m.id }))
