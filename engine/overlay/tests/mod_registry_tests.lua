...
-- ------- cross-mod register collision: the first owner keeps the id and
-- the loser's mod still loads.  The device shape this pins: a follower mod
-- (api 1) patched icon records for every Gen 1 species, and CRYSTAL_251's
-- plain icons:register("BULBASAUR") then errored and took the whole 251-mon
-- overhaul down with it.  A cosmetic collision must degrade to a warning,
-- not kill the losing mod's load.
local collideFiles = {
  -- api 1, patching with extra fields, exactly like the follower mod
  ["mods/first_pack/manifest.json"] = manifestJson("first_pack"),
  ["mods/first_pack/main.lua"] = [[
return function(mod)
  mod.content.icons:patch("PIKACHU",
    { image = "first_pack/icon.png", width = 16, height = 16, frames = 1 })
  mod.content.items:register("SHARED_ITEM",
    { id = "SHARED_ITEM", name = "FIRST", price = 1 })
end
]],
  -- api 2, registering over both of first_pack's claims plus one of its own
  ["mods/second_pack/manifest.json"] = manifestJson("second_pack",
    { api = 2, deps = '["first_pack"]' }),
  ["mods/second_pack/main.lua"] = [[
return function(mod)
  mod.content.icons:register("PIKACHU", "QUADRUPED")
  mod.content.items:register("SHARED_ITEM",
    { id = "SHARED_ITEM", name = "SECOND", price = 2 })
  mod.content.items:register("SECOND_ITEM",
    { id = "SECOND_ITEM", name = "SECOND ITEM", price = 3 })
end
]],
}
local collideData = fixtureData()
local collideLoader = Loader.new({ fs = memfs(collideFiles) })
check(collideLoader:load(collideData) == true,
  "a cross-mod register collision no longer fails the load")
check(collideLoader.mods.second_pack.state == "loaded",
  "the colliding mod still loads")
local pikaIcon = collideLoader.content.icons:get("PIKACHU")
check(type(pikaIcon) == "table" and pikaIcon.image == "first_pack/icon.png",
  "the first owner's icon record survives the later register")
check(collideData.icons and collideData.icons.bySpecies
  and collideData.icons.bySpecies.PIKACHU
  and collideData.icons.bySpecies.PIKACHU.image == "first_pack/icon.png",
  "the merged icon target keeps the first owner's record")
check(collideLoader.content.items:get("SHARED_ITEM").name == "FIRST",
  "register-vs-register keeps the first owner's value too")
check(collideData.items.SECOND_ITEM ~= nil
  and collideData.items.SECOND_ITEM.price == 3,
  "the loser's non-colliding content still applies")
check(logged("[second_pack]", "icons already registered: PIKACHU"),
  "the skipped register warns, attributed to the losing mod")
check(logged("icons already registered: PIKACHU", "keeping first_pack's value"),
  "the warning names the owner that kept the id")

-- registering over the base table (vanilla content) is still a load error:
-- the explicit-override contract only softens between two mods
local baseCollideFiles = {
  ["mods/base_clobber/manifest.json"] = manifestJson("base_clobber", { api = 2 }),
  ["mods/base_clobber/main.lua"] = [[
return function(mod)
  mod.content.items:register("POTION", { id = "POTION", name = "POTION", price = 1 })
end
]],
}
local baseCollideData = fixtureData()
local baseCollideLoader = Loader.new({ fs = memfs(baseCollideFiles) })
check(baseCollideLoader:load(baseCollideData) == false,
  "registering over vanilla content still fails the mod")
check(table.concat(baseCollideLoader.errors, "\n")
  :find("items already registered: POTION", 1, true) ~= nil,
  "the base collision keeps the explicit-override error")
check(baseCollideData.items.POTION.price == 300,
  "the failed register leaves the vanilla record untouched")

-- a mod colliding with itself is a deterministic authoring bug: still fatal
local selfCollideFiles = {
  ["mods/self_dup/manifest.json"] = manifestJson("self_dup", { api = 2 }),
  ["mods/self_dup/main.lua"] = [[
return function(mod)
  mod.content.items:register("DUP_ITEM", { id = "DUP_ITEM", name = "ONE", price = 1 })
  mod.content.items:register("DUP_ITEM", { id = "DUP_ITEM", name = "TWO", price = 2 })
end
]],
}
local selfCollideLoader = Loader.new({ fs = memfs(selfCollideFiles) })
check(selfCollideLoader:load(fixtureData()) == false,
  "a mod registering the same id twice still fails")

-- the owner matrix at registry level: engine-owned and ownerless content
-- stay strict, and the cross-mod skip hands back nil rather than a value
local matrixReg = Registry.new("icons", Schemas.REGISTRIES.icons)
matrixReg:register("MEWMON", "BALL", Schemas.ENGINE)
check(not pcall(function() matrixReg:register("MEWMON", "BIRD", "some_mod") end),
  "registering over engine-owned content still errors")
local bareReg = Registry.new("things")
bareReg:register("KEY", "first")
check(not pcall(function() bareReg:register("KEY", "second") end),
  "ownerless v1 registries keep the strict duplicate error")
local skipReg = Registry.new("icons", Schemas.REGISTRIES.icons)
skipReg:register("SKIPMON", "BALL", "mod_a")
skipReg:register("SKIPMON2", "BALL", "mod_a")
skipReg:register("SKIPMON3", "BALL", "mod_a")
local skipResult
check(pcall(function() skipResult = skipReg:register("SKIPMON", "BIRD", "mod_b") end),
  "a cross-mod duplicate register no longer raises")
check(skipResult == nil, "the skipped register returns nil")
check(skipReg:get("SKIPMON") == "BALL", "the first owner's value stands")

-- a mod losing a whole table of ids logs one attributed line plus a
-- freeze-time tally, not one line per id
skipReg:register("SKIPMON2", "BIRD", "mod_b")
skipReg:register("SKIPMON3", "BIRD", "mod_b")
check(loggedCount("[mod_b]", "icons already registered:") == 1,
  "repeat skips against one owner log a single full line")
skipReg:freeze()
check(logged("[mod_b]", "2 more ids kept mod_a's values"),
  "freeze summarizes the tallied skips")
check(skipReg:get("SKIPMON2") == "BALL" and skipReg:get("SKIPMON3") == "BALL",
  "every tallied id keeps the first owner's value")
